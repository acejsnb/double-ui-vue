import {
    defineComponent, reactive, onMounted, watch
} from 'vue';
...
const SelectView = defineComponent({
    ...
    setup() {
        const state = reactive({
            ...
            sid2: 0,
            selectData,
            selectData2: JSON.parse(JSON.stringify(selectData2)),
            mid: ['222', '444'],
            selectData3,
            errorText: '错误提示',
            errorShow: false
        ...
        const change3 = (ids, items) => {
            console.log('change3:::', ids, items);
        };

        watch(() => state.sid2, (n, o) => {
            console.log('watch::', n, o);
        });

        return () => {
            const {
                sid2, selectData2, errorText, errorShow,
                mid, selectData3
            } = state;

            return (
                <div class="mSelect">
                    <h3 className="components-title-h3">width + caption</h3>
                    <Select
                        width="180"
                        v-model={state.sid2}
                        data={selectData2}
