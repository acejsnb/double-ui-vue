...
const DrawerView = defineComponent({
	...
	setup() {
		...
		return () => {
			const { drawerStatus, loadingMore, loading, height } = state;
			return (
				<div class="component component-padding">
					<h1 class="components-title">Drawer组件</h1>
					<button onClick={drawerShow}>drawerShow</button>
					<button onClick={closeLoading}>打开关闭loading</button>
					<Drawer
						show={drawerStatus}
						bottom={true}
						focus={false}
						...
						v-slots={{
							...
							handle: () => (
								<div>
									<button onClick={drawerConfirm}>确定</button>
									<button onClick={drawerCancel}>取消</button>
								</div>
							)
						}}
					/>
