 ...
 * @return boolean
 */
const SetCheckboxShow = (item, thisObj) => {
    const {
 ...
 * @return string
 */
const SetCheckedStatus = ({ id }, thisObj) => {
    const { value, multiple } = thisObj;
    if (multiple) {
 ...
 * @return []
 */
const TileTool = async (result, tree, pid, index = null, open = false, thisObj) => new Promise(resolve => {
    const { omit } = thisObj;
    tree.forEach((d, i) => {
        const ind = index ? `${index}-${i}` : `${i}`,
            ...
            obj = {
                ...
                ...{
                    index: ind,
                    parentId: pid,
                    checked: SetCheckedStatus(d, thisObj),
                    open: !!d.open,
                    ...
                    tag, // 标签
                    // 是否显示checkbox
                    showCheckbox: SetCheckboxShow(d, thisObj)
                }
            };
        delete obj.children;
        result.push(obj);
        if (d.children && d.children instanceof Array && JSON.stringify(d.children).length > 4) {
            TileTool(result, d.children, d.id, ind, d.open, thisObj);
        }
    });
    resolve(result);
});
...
const SetParentStatusById = ({
    parentId, tileData, checkedData, thisObj
}) => {
    const { notNull, includeParent } = thisObj;
    tileData.forEach(d => {
 ...
 * @constructor
 */
const GetCheckedData = (tileData, thisObj) => {
    const {
...
const FilterCheckedData = ({
    tileData, checkedData, status, item, thisObj
}) => {
    const {
 ...
 * @constructor
 */
const SetChecked = (item, tileList, thisObj) => {
    const {
 ...
 * @constructor
 */
const SetCheckedSortByTree = (item, tileData, thisObj) => {
    const {
    ...
    const checkedData = GetCheckedData(tileData, thisObj);
    const checkedIds = checkedData.map(d => d.id);
    return { tileData, checkedIds, checkedData: JSON.parse(JSON.stringify(checkedData)) };
};
 ...
 * @return string
 */
const SetTileCheckedInit = async (tile, thisObj) => new Promise((resolve) => {
    const {
    ...
    if (multiple && value instanceof Array) {
        ...
        if (childDisable) {
            ...
            items.forEach(d => {
                ...
                // 设置父项数据
                if (linkage) SetParentStatusById({ parentId: d.parentId, tileData, thisObj });
            });
        }
    }
    if (multiple) {
        const checkedData = GetCheckedData(tile, thisObj);
        const checkedIds = checkedData.map(d => d.id);
...
const StrategyChange = (status, { id }, thisObj) => {
    const { tileList } = thisObj;
    const strategy = {
        // 选择所有子级
        allChild() {
    ...
    const tileData = strategy[status](),
        checkedData = GetCheckedData(tileData, thisObj),
