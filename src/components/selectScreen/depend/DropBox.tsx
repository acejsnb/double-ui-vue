	...
	setup(props) {
		...
		const cancel = () => {
			...
			if (status.includes('multiple'))
				state.dropData = JSON.parse(JSON.stringify(dropDataHistory));
				...
				<Transition name={position ? 'selectDownUpExtend' : 'selectDownUpExtendTop'}>
					...
					>
						{status.includes('Search') && (
							<div class="p-drop-box-search">
								<input
									type="text"
									class="p-drop-box-input"
									value={searchText}
						...
						{status.includes('multiple') && (
							<div
								class={[
									'p-drop-box-handle',
									dropData.length > 5 && 'p-drop-box-handle-shadow'
								]}
							>
								<section class="p-drop-box-btn p-drop-box-cancel" onClick={cancel}>
									取消
								</section>
								<section
									class={[
										'p-drop-box-btn',
										activeConfirm ? 'p-drop-box-confirm' : 'p-drop-box-disabled'
									]}
									onClick={confirm}
								>
									确定
