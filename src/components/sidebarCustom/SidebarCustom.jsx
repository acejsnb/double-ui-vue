...
import {
    defineComponent, reactive, watch, onMounted, onBeforeUnmount, getCurrentInstance
} from 'vue';
...
const SidebarCustom = defineComponent({
    ...
    setup(props, { emit, slots }) {
        ...
        watch(() => props.show, (n, o) => {
            if (n === o) return;
            if (n) state.leftWidth = state.width || state.leftWidthHistory;
        ...
        const closeLeft = () => {
            if (state.leftWidth) {
                state.leftWidth = 0;
                setTimeout(() => { state.leftShow = false; }, 300);
            } else {
                state.leftShow = true;
                setTimeout(() => { state.leftWidth = state.leftWidthHistory; });
            }
            setTimeout(() => { emit('change', state.leftWidth, state.leftShow); }, 300);
        };

        return () => {
            const { width, openClose, changeSize } = props,
                {
                    pointX, left, leftWidth, windowWidth, leftShow
                } = state;
            return (
                <div class={[
                    'p-sidebar-custom',
                ...
                onMouseup={lineUp}
                >
                    <>{pointX > 0 && <div class="p-sidebar-custom-line" style={{ left: `${pointX - left - 1}px` }}/>}</>
                    <>{width && (
                        <div class={{ 'p-sidebar-custom-bar': true, 'p-sidebar-custom-bar-change-size': changeSize && leftWidth }}
                            style={{ left: `${leftWidth - 2}px` }}
                            onMousedown={lineDown}
                        />
                    )}</>
                    <>{openClose && (
                        <div class={['p-sidebar-custom-btn', leftWidth ? 'p-sidebar-custom-btn-normal' : 'p-sidebar-custom-btn-rotate']}
                            style={{ left: `${leftWidth || (windowWidth >= 1920 ? 22 : 20)}px` }}
                            onClick={closeLeft}
                        >
                            <ArrowDoubleLeft/>
                        </div>
                    )}</>
                    <div v-show={leftShow} class="p-sidebar-left" style={{ width: `${leftWidth}px` }}>
                        {slots.left()}
                    </div>
