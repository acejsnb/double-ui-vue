...
const OptionItem = (props: OItem) => (
	<section
		...
		class={[
			'p-drop-option-item',
			props.value === props.id && 'p-drop-option-selected',
			props.disabled && 'p-drop-option-disable'
		]}
		data-id={props.id}
	>
		{props.icon && <i class="p-drop-option-svg" />}
			...
			return (
				<Transition name={position ? 'selectDownUpExtend' : 'selectDownUpExtendTop'}>
					...
					>
						{openSearch && (
							<span
								class={['p-drop-search', scrollTop > 12 && 'p-drop-search-shadow']}
							>
								<input
									class="p-drop-input"
									type="text"
									...
									onInput={inputHandle}
								/>
								<i v-show={inputVal} class="p-drop-clear" onClick={clearInput}>
									<ClearSvg />
								</i>
							</span>
						)}
