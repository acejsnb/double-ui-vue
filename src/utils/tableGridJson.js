...
const CreateJson = (num) => {
    const arr = [];
    for (let i = 0; i < num; i++) {
        arr.push({
            id: String(i),
            name: { text: `青阳万达广场${i}` },
            date: { text: `20${RandomNum(10, 19)}-${i % 2 ? '0' : '1'}${i % 2 ? RandomNum(0, 9) : RandomNum(0, 2)}-${RandomNum(0, 2)}${RandomNum(1, 9)}`.replace(/-/g, '.') },
            strategyStatus: { text: strategyStatus[RandomNum(0, 3)] },
            strategyTime: { text: `${RandomNum(0, i % 2 ? 2 : 1)}${RandomNum(0, i % 2 ? 3 : 9)}:${RandomNum(0, 5)}${RandomNum(0, 9)}` },
            executeTime: { text: `${RandomNum(0, i % 2 ? 2 : 1)}${RandomNum(0, i % 2 ? 3 : 9)}:${RandomNum(0, 5)}${RandomNum(0, 9)}` },
